...
# - Let's visualize the distribution of rivers_km with a histogram using the 
#   hist() function.
...
# - numeric (real numbers): e.g., 1.2, 10.7, 5.91
# - integer (whole numbers): 1, 2, 3
# - character (text or string data): "apple", "pear", "orange"
# - logical (TRUE or FALSE): "TRUE", FALSE". Very important in R!
...
# Pipes organize operations sequentially onto different lines for readability.

#--------------------------------------
# dplyr: filter() and select()
#--------------------------------------

# select just the mpg column as a 1 column dataframe
mtcars |>
     select(mpg)

# select the mpg column for cars with 6 cylinders
mtcars |>
     filter(cyl == 6) |>
     select(mpg)

## Practice
# Use filter() and select() to find the horsepower (hp) of cars with 4 cylinders
...
# create a new column, mpg_per_cyl, and calculate the average
# Note: mutate() creates a new column and summarize() creates a summary table
...
# group_by() groups a data frame into smaller subsets for use with summarize() 
# to create conditional statistics 

# Here is average mpg by cylinder:
...
## Practice
# 1. What is average car weight by gear?


# 2. What is average and median car weight and mpg by cylinder?
...
# ggplot organizes plots by layers connected with "+"

# 1. blank canvas

ggplot()

# 2. add data and aesthetic mappings (x and y coordinates) 

ggplot(data = mtcars, 
       aes(x = wt, y = mpg)) # uses aes()

# 3. add plot type -- geom_point for scatterplot

ggplot(data = mtcars, 
       aes(x = wt, y = mpg)) +
     geom_point()

# 4. add title and axis labels with labs() 
...
# 5. Add a third dimension with a color element. Use "col =" within aes()
# Note: for color to be discrete we need to change it into a factor.
